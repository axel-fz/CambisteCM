 ...
 * Client dashboard section — uses TanStack Query (useQuery) to load
 * counterpart changers, with stale-while-revalidate caching.
 */
"use client";
...
async function fetchChangers(currency: string): Promise<ChangerCardData[]> {
  const query = currency ? `?currency=${encodeURIComponent(currency)}` : "";
  const res = await fetch(`/api/listings/for-me${query}`, { cache: "no-store" });
  if (!res.ok) throw new Error("Impossible de charger les profils disponibles.");
  return res.json() as Promise<ChangerCardData[]>;
}
...
export default function ChangersSection({ role }: ChangersSectionProps) {
  ...
  const [selectedChanger, setSelectedChanger] = useState<ChangerCardData | null>(null);
  ...
  const {
    ...
    queryKey: ["changers", currency],
    queryFn: () => fetchChangers(currency),
  });
  ...
  const sectionTitle =
    role === "echangeur" ? "Changeurs Pro disponibles" : "Échangeurs disponibles";
  const currencyLabel =
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_180px]">
          ...
          <label className="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3">
            <span className="material-symbols-outlined text-slate-400">currency_exchange</span>
            <input
              value={currency}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-slate-800">{changer.user.name}</h3>
                    <p className="text-sm text-slate-500">
              ...
              <div className="mt-6 space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-800">{changer.currency}</p>
                </div>
                <div>
                  <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Taux</p>
